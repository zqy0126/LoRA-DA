...
def initialize_text_to_text_model(
    ...
    if model_type == "CausalLM":
        if flash_attention and importlib.util.find_spec("flash_attn") is not None:
            ...
            if flash_attention:
                log.warning("flash-attn is unavailable; falling back to PyTorch SDPA")
            model = AutoModelForCausalLM.from_pretrained(
                model_name,
                trust_remote_code=True,
                torch_dtype=torch.bfloat16 if bf16 else torch.float32,
...
def train_text_to_text_model(
    ...
    **kwargs,
    ...
    training_args = TrainingArgumentsClass(
        ...
        save_steps=kwargs.get("save_steps", eval_steps),
        save_strategy=kwargs.get("save_strategy", "epoch"),
        save_total_limit=kwargs.get("save_total_limit", kwargs.get("eval_epochs", 1)),
